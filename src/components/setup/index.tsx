...
export default function SetupPage({ profile }: SetupPageProps) {
  ...
  if (profile.storage?.length === 0) {
    return <ErrorDetails error={new Error("No valid storage listed in profile")} />;
  }

  const onSubmit = handleSubmit(async (form) => {
    if (!profile.storage?.[0]) return;

    const todoDataset = createLdoDataset(getDataset(profile.storage?.[0]));
      ...
      <TextContent>
        {isSubmitSuccessful && list ? (
          <>
            ...
            <SubmitButton href={list ? `/list/${encodeURIComponent(list["@id"]!)}` : "/"}>Let&#39;s get to your first
              todo list!</SubmitButton>
          </>
        ) : (
